let batch =`pid:827837505 byr:1976
hgt:187cm
iyr:2016
hcl:#fffffd
eyr:2024
...
const batch2 = `peyr:1972 cid:100
...
const batchArr2 = batch2.split('\n\n');


function objectify(arr){
    let newArr = []; 
    arr.forEach(ele => {
        let passportbatch = ele.split(/\s+/);
       newArr.push(passportbatch)
    })
    let arrOfObject =[];
    newArr.forEach(e => {
        let passObject = {};
        e.forEach(data => {
           ...
           passObject[keyValuePair[0]] = keyValuePair[1];
        })
        arrOfObject.push(passObject); 
...
let arrObj = objectify(batchArr)

function checkValidPassport(arrObj){
    let countValid = 0 ;
    arrObj.forEach(passport => {
        console.log(passport)
        if (passport.byr && passport.iyr && passport.eyr && passport.hgt && passport.hcl && passport.ecl && passport.pid){
            let byr = passport.byr.length === 4 && passport.byr >= 1920 && passport.byr <= 2002 ? true : false;
            let iyr = passport.iyr.length === 4 && passport.iyr >= 2010 && passport.iyr <= 2020 ? true : false;
            let eyr = passport.eyr.length === 4 && passport.eyr >= 2020 && passport.eyr <= 2030 ? true : false;
            let ecl = passport.ecl === 'amb' || passport.ecl === 'blu' || passport.ecl === 'gry' || passport.ecl === 'grn' || passport.ecl === 'hzl' || passport.ecl === "oth" || passport.ecl === "brn" ? true : false;
            let arrPid = passport.pid.split("");
            let pid = false;
            if(arrPid.length === 9)
             pid = arrPid.every(char => {
                return !isNaN(char);
            });
            let arrHcl =passport.hcl.split("");
            let hcl = false;
            if(arrHcl[0] === "#"){
                let checkHcl = 0;
                for(i=1 ; i < arrHcl.length ; i++){
                    if(arrHcl[i] >= 0 && arrHcl[i] <= 9 ){
                        checkHcl++;
                    }
                    if(arrHcl[i].toUpperCase()==='A' || arrHcl[i].toUpperCase() === 'B' || arrHcl[i].toUpperCase() === 'C'|| arrHcl[i].toUpperCase() === 'D'|| arrHcl[i].toUpperCase() === 'E'|| arrHcl[i].toUpperCase() === 'F'){
                        checkHcl++;
                    }
                }
                if(checkHcl === 6){
            ...
            let arrHgt = passport.hgt.split("");
            let hgt = false;
            let mesaureUnit = arrHgt[arrHgt.length-2]+arrHgt[arrHgt.length-1];
            let heigh = 0;
            if(mesaureUnit.toUpperCase() === 'CM' && arrHgt.length === 5){
                 heigh = arrHgt[0]+arrHgt[1]+arrHgt[2];
                if( parseInt(heigh) >= 150 && parseInt(heigh) <= 193){
                    hgt =true;
                }

            }
            if(mesaureUnit.toUpperCase() === 'IN' && arrHgt.length === 4){
                 heigh = arrHgt[0]+arrHgt[1];
                if( parseInt(heigh) >= 59 && parseInt(heigh) <= 76){
                    hgt =true;
                }
